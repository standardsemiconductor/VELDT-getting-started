...
```
Next, edit the `veldt.cabal`. We remove the executable section and add our own library section. We leave everything else untouched. You may customize attributes like `synopsys` but it is not necessary. Your `veldt.cabal` file should look similar:
```
cabal-version:       >=1.10
